...
Managing SCSS assets in Odoo is not as straightforward as it is in some other environment, but
it's highly efficient.

Modularity is key. The inheritance scheme described further allows Odoo to:

- customize Bootstrap CSS framework;
...
"Direct variables’ overrides" are technically possible in SCSS but may lead to inconsistent
results in complex environments like Odoo.
...
When using the `!default` flag, the compiler assigns a value **only** if that variable isn't
defined yet.

As a result of this technique, the priority of how variables are assigned matches the assets'
loading order.
...
.. important::
   | This diagram is incomplete and doesn't match the current bundles' organization.
