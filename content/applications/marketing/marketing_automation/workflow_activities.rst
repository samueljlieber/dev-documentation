...
be single workflow in each campaign, however, a workflow can be made up of any number of
...
Automation --> Campaigns` dashboard, then clicking the :guilabel:`Add new activity` button. Doing so
opens the :guilabel:`Create Activities` modal.
...
:guilabel:`Create Activities` modal to add another activity. Clicking :guilabel:`Discard` closes the
modal without saving the activity.

.. image:: workflow_activities/create-activities.png
   :align: center
   :alt: The create activities modal.
...
There are three different types of activities available in Marketing Automation:
...
If :guilabel:`Email` is selected as the :guilabel:`Activity Type`, the option to :guilabel:`Pick or
Create a Template...` in the :guilabel:`Mail Template` field is available.

To create a new template directly from the :guilabel:`Mail Template` field, start typing the title
of the new template and select :guilabel:`Create and edit...` to reveal a :guilabel:`Create
Marketing Template` modal. Proceed to create and configure the new email template.
...
return to the :guilabel:`Create Activities` modal and continue to configure the :ref:`trigger
...
modal and continue to configure the :ref:`trigger <marketing_automation/trigger>`.

.. tip::
   To view all server actions in the database, activate :ref:`developer-mode` and navigate to
   :menuselection:`Settings --> Technical --> Actions --> Server Actions` dashboard.
...
reveals a blank :guilabel:`Create Server Action` modal, wherein a custom server action can be
created and configured.

.. image:: workflow_activities/create-sa.png
   :align: center
   :alt: The Create Server Action modal.

On the :guilabel:`Create Server Action` modal, select the :guilabel:`Type` of server action, the
configuration fields change depending on the type selected:

- :guilabel:`Update a Record`: update the values of a record.
...
Once the server action is configured, click :guilabel:`Save & Close` to save the activity and return
to the :guilabel:`Create Activities` modal and continue to configure the :ref:`trigger
<marketing_automation/trigger>`.
...
If :guilabel:`SMS` is selected as the :guilabel:`Activity Type`, the option to :guilabel:`Pick or
Create a Template...` in the :guilabel:`SMS Template` field is available.
...
Template` modal. Proceed to create and configure the new SMS template.
...
the :guilabel:`Create Activities` modal is to define when the activity should be executed. This is
done in the :guilabel:`Trigger` field group.

.. image:: workflow_activities/trigger.png
   :align: center
   :alt: The trigger field group on Create Activities modal.
...
Optionally, an :guilabel:`Expiry Duration` can be defined in the :guilabel:`Create Activities` modal
to cancel the execution of this activity after a specific amount of time. Selecting this checkbox
reveals the :guilabel:`Cancel after` field with *interval* and *interval type* inputs.
...
The :guilabel:`DOMAIN` section of the :guilabel:`Create Activities` modal contains fields to further
filter the target audience of the activity.
...
campaign, and the fields that are available to filter by are also specific to the :guilabel:`Target`
of the campaign.
...
activity` button beneath the desired parent activity.
...
Once a trigger type is selected, the :guilabel:`Create Activities` modal opens to configure the
child activity. The process is the same as :ref:`creating a new activity
<marketing_automation/activities>`, with the exception that the :guilabel:`Trigger` field is
pre-filled with the selected trigger type and the and :guilabel:`Activity` field has the parent
activity selected.
